...
// Spotify config for release checking
...
// Check if track is on Spotify
async function isOnSpotify(artist: string, title: string): Promise<{ found: boolean; url?: string }> {
...
// Check for similar track already in bucket (fuzzy match)
async function isDuplicateInBucket(
  artist: string,
  title: string,
  duration: number,
  supabase: any
...
async function scrapeAndUpload(url: string, config: ScraperConfig): Promise<void> {
  ...
  for (const track of tracks) {
    ...
    const dupCheck = await isDuplicateInBucket(artist, title, track.duration, supabase);
    ...
    const spotifyCheck = await isOnSpotify(artist, title);
